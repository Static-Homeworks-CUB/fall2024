...
export class SingleLetterIdentifier extends ASTDetector {
    ...
    private checkExternals(cu: CompilationUnit): void {
        ...
        const receivers = contracts.flatMap(contract => contract.declarations.filter(decl => decl.kind == "receiver")) as AstReceiver[];
        for (const receiver of receivers) {
            const externalReceiverSelectorKinds = ["external-simple", "external-fallback", "external-comment"];
